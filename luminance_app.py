...
class ProcessThread(QThread):
    ...
    def _apply_rotation_to_map(self, y_map):
        if y_map is None or not bool(self.options.get('rotation_enabled', False)):
            return y_map
        angle_idx = int(self.options.get('rotation_angle', 0))
        if angle_idx == 0:
            return np.rot90(y_map, k=3)
        if angle_idx == 1:
            return np.rot90(y_map, k=2)
    ...
    def _load_flatfield_luma_norm(self, grey_path, demosaic, apply_dark_level, dark_frame_offset,
        ...
        # Keep flat-map photometry in the same domain and orientation as input maps before division.
        flat_y = self._apply_linearity_lut(flat_y)
        flat_y = self._apply_grey_rotation_to_map(flat_y)
        return self._apply_rotation_to_map(flat_y)
    ...
    def run(self):
        ...
        try:
            ...
            def _prepare_phase1_sensor_norm(filepath):
                ...
                sensor_norm = self._apply_rotation_to_map(sensor_norm)
            ...
            if bool(self.options.get('rotation_enabled', False)):
                print(f"Image rotation (pre-processing): enabled ({self.get_rotation_label(int(self.options.get('rotation_angle', 0)))})")
            else:
                print("Image rotation (pre-processing): disabled")
            ...
            for i, (f, out_path) in enumerate(out_paths):
                ...
                Y_norm = self._apply_rotation_to_map(Y_norm)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._light_compensation_in_progress = False
    ...
    def initUI(self):
        ...
        left_panel_layout.addWidget(self.main_scroll_area, 1)

        console_panel = QFrame()
        console_panel.setFrameShape(QFrame.StyledPanel)
        console_panel.setMinimumWidth(0)
        console_panel_layout = QVBoxLayout(console_panel)
        console_panel_layout.setContentsMargins(0, 0, 0, 0)
        console_panel_layout.setSpacing(6)
        ...
        dark_toggle = self._create_section_toggle("4) Dark Calibration", dark_box, default_open=False)
        ...
        linearity_lut_btn_row.setContentsMargins(0, 0, 0, 0)
        ...
        linearity_toggle = self._create_section_toggle("5) Sensor Linearity Calibration", linearity_box, default_open=False)
        ...
        ds_widget.setVisible(False)
        clip_controls.addWidget(ds_widget)
        ...
        phase1_toggle = self._create_section_toggle("7) Stretch Analysis", phase1_box, default_open=True)
        ...
        self.check_rotation_enable = QCheckBox("Enable rotation before processing/export")
        ...
        rotation_toggle = self._create_section_toggle("3) Image rotation", self.rot_widget, default_open=False)
        ...
        grey_top_row.addSpacing(10)
        grey_top_row.addWidget(QLabel("ROI size (% short side):"))
        ...
        self.spin_grey_roi_short_side_pct.setToolTip("ROI size for grey-card calibration as percentage of the image short side (1% to 10%).")
        ...
        flatfield_row.addSpacing(10)
        flatfield_row.addWidget(QLabel("Flatfield smooth sigma (% short side):"))
        ...
        flatfield_row.addWidget(self.spin_flatfield_sigma_pct)
        ...
        grey_toggle = self._create_section_toggle("6) Grey Card Calibration", self.grey_box, default_open=False)
        ...
        rot_insert_index = layout.indexOf(output_box) + 1
        layout.insertWidget(rot_insert_index, rotation_toggle)
        layout.insertWidget(rot_insert_index + 1, self.rot_widget)
        ...
        console_panel_layout.addWidget(QLabel("Python Console Log"))
        console_panel_layout.addWidget(console_box, 1)

        left_panel_layout.addWidget(execution_box, 0)
    ...
    def _flatfield_lookup_for_current_inputs(self):
        return self._build_flatfield_map_for_inputs()
    ...
    def _preview_apply_flatfield(self, input_path, y_norm, dark_ctx=None):
        ...
        flat_map, err = self._flatfield_lookup_for_current_inputs()
        if err:
            return y_norm
    ...
    def _preview_grey_luma_norm_map(self, filepath, dark_ctx=None, target_shape=None):
        ...
        if ext in raw_exts:
            ...
            y = self._apply_rotation_to_map(y)
            if post_ds > 1:
                y = y[::post_ds, ::post_ds]
        else:
            y = self._read_linear_luma_norm_for_grey(filepath, dark_ctx=ctx)
    ...
    def _compute_light_compensation_payload(self, patch_size=20, progress_callback=None, cancel_callback=None):
        if not self.files:
    ...
    def update_grey_calibration_ui_state(self):
        ...
        self.btn_add_grey_frames.setEnabled(True)
        self.btn_clear_grey_frames.setEnabled(True)
        self.grey_frames_list.setEnabled(True)
        ...
        self.spin_flatfield_sigma_pct.setEnabled(self.check_flatfield_enable.isChecked())
    ...
    def save_led_intensity_compensation(self):
        ...
        self._calculation_cancel_requested = False
        self._light_compensation_in_progress = True
        self._set_light_compensation_ui_running(True)

        try:
            payload, gain_map = self._compute_light_compensation_payload(
                progress_callback=self._light_compensation_progress,
                cancel_callback=lambda: self._calculation_cancel_requested
            )
    ...
    def _gray_map_to_pixmap(self, y_map, width=178, height=128):
        if y_map is None or y_map.size == 0:
            return QPixmap()
        y = np.clip(y_map.astype(np.float32), 0.0, None)
    ...
    def update_flatfield_preview_panel(self):
        ...
        try:
            ...
            flat_map, flat_err = self._flatfield_lookup_for_current_inputs()
            flat_y = None
            use_paired = bool(flat_map) and flat_err is None
            if use_paired:
            ...
            if flat_y is not None:
                ...
                self.grey_flat_frame_label.setPixmap(QPixmap())
                self.grey_flat_frame_label.setText("Self ref" if self.check_flatfield_enable.isChecked() else "No flat")

            pair_txt = os.path.basename(filepath)
            if self.check_flatfield_enable.isChecked() and not use_paired:
                self.lbl_flatfield_preview_info.setText(f"{pair_txt} | self-reference fallback")
            elif flat_err:
    ...
    def _build_stack_cache_key(self, mode):
        ...
        return (
            ...
            self.check_rotation_enable.isChecked(),
            self.combo_rotation.currentIndex(),
        )
    ...
    def _light_compensation_progress(self, done, total):
        denom = max(1, int(total))
        pct = int((int(done) / denom) * 100)
        self.progress.setValue(max(0, min(100, pct)))
        QApplication.processEvents()

    def _is_calculation_busy(self):
        return bool(self._light_compensation_in_progress) or bool(self.thread is not None and self.thread.isRunning())

    def _request_calculation_stop(self):
    ...
    def _set_calculation_idle(self):
        self._light_compensation_in_progress = False
        self._calculation_cancel_requested = False
        self.set_button_develop()
    ...
    def _resolve_light_compensation_for_processing(self, progress_callback=None, cancel_callback=None):
        if not self.check_calibrate_light_variance.isChecked():
            return True, {}, None
        ...
        try:
            ...
            return False, {}, f"No valid compensation file found and unable to compute from grey cards. Reason: {e}"
        except Exception as e:
            return False, {}, (
    ...
    def start_processing(self, phase1_only=False):
        ...
        if options.get('flatfield_enabled', False):
            flat_map, flat_err = self._flatfield_lookup_for_current_inputs()
            if flat_err:
                print(f"Flatfielding disabled for this run: {flat_err}")
                options['flatfield_map'] = {}
            else:
                if flat_map:
                    print(f"Flatfielding: paired grey-card reference ({len(flat_map)} input-grey pairs)")
                options['flatfield_map'] = flat_map

        if options.get('light_compensation_enabled', False):
            self.lbl_status.setText("Preparing light compensation...")
            self.progress.setValue(0)
        ...
        self._light_compensation_in_progress = False
        if not ok_light:
            self._set_calculation_idle()
            QMessageBox.warning(self, "Grey Card Calibration", light_payload)
            return
        if self._calculation_cancel_requested:
            self._set_calculation_idle()
            self.progress.setValue(0)
            self.lbl_status.setText("Ready")
            return
